...
Execution of a(8): 3 (val of y in line 1) + b(8) --> 8 >= 6 so 8 - 7 = 1 --> prints 1 and returns that for b(8). x is referencing the value 3+1 = 4.

Repeat the same process for y = a(b(x)). Since x is now 4, y = a(b(4)). b(4) --> 4 < 6 --> 4 - 4 = 0 print and return 0. a(0) --> 3 (1st line) + b(0). b(0) --> 0<6 --> 4 - 0 = 4, print and return 4. a(0) returns 7. y is now referencing the value 7
...
Since we are checking if string1 is a subsequence of string 2, and the code wants to check based on the last character of string 1. Backward tracing of strings

To contextualise, the ideal execution based on last character checking, I will be using cs/sc and c10s10.

## Sample execution

<details>
<summary><b>Sample execution (cs, c10s10)</b></summary>
...
Compare: string1[1]='s' vs string2[5]='0'
No match → Move n backward
```
...
Compare: string1[1]='s' vs string2[4]='1'
No match → Move n backward
```
...
Compare: string1[1]='s' vs string2[3]='s'
Match! ✓ → Move both backward
```
...
Compare: string1[0]='c' vs string2[2]='0'
No match → Move n backward
```
...
Compare: string1[0]='c' vs string2[1]='1'
No match → Move n backward
```
...
Compare: string1[0]='c' vs string2[0]='c'
Match! ✓ → Move both backward
```
...
<summary><b>Sample execution (sc, c10s10)</b></summary>
...
Compare: string1[1]='c' vs string2[5]='0'
No match → Move n backward
```
...
Compare: string1[1]='c' vs string2[4]='1'
No match → Move n backward
```
...
Compare: string1[1]='c' vs string2[3]='s'
No match → Move n backward
```
...
Compare: string1[1]='c' vs string2[2]='0'
No match → Move n backward
```
...
Compare: string1[1]='c' vs string2[1]='1'
No match → Move n backward
```
...
Compare: string1[1]='c' vs string2[0]='c'
Match! ✓ → Move both backward
```
...
string2:  c  1  0  s  1  0
          ↑ (n=0, no more characters)

n == 0 and m > 0 → Return False ✗

We found 'c' but ran out of string2 before finding 's'
```

</details>

Claude so goated for helping me to create the pointers


Based on the logic above, we know that m is 0, it is a subsequence so the error must be at lines 3 and 5. Swap the 2 lines and change the 2nd last return line to n-1 the algorithm is correct
